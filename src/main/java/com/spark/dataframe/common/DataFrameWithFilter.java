...
public class DataFrameWithFilter {
	final static Logger logger = Logger.getLogger(DataFrameWithFilter.class);
    ...
    public static void main( String[] args )throws Exception {
    	spark = SparkSession.builder().master("local").appName("MongoSparkConnectorIntro")
    		    ...
    		    Dataset<Row> filterDataSet = dataSet1.filter(dataSet1.col("Marks").isin(numList.stream().toArray()));
        	    filterDataSet.show();
    		    jsc.close();
    }
